...
def fast_calculate_causal_scores_DAG(data: pd.DataFrame, nodes: list[str], edges: list[(str, str)], outcome: str, estimator=augmented_ipw, l=0.5):
    causal_score = {}
    G = CausalDAG(nodes, edges)
    treatment_nodes = [x for x in nodes if x != outcome]
    
    for node in treatment_nodes:
        minop_set = G.find_optimal_backdoor(node, outcome)
        if minop_set is None:
            causal_score[node] = -1.0
...
def fast_calculate_causal_scores_ADMG(data: pd.DataFrame, nodes: list[str], di_edges: list[(str, str)], bi_edges: list[(str, str)], outcome: str, estimator=augmented_ipw, l=0.5):
    causal_score = {}
    G = CausalADMG(nodes, di_edges, bi_edges)
    treatment_nodes = [x for x in nodes if x != outcome]
    
    for node in treatment_nodes:
        minval_out = G.find_valid_adjustment_set(node, outcome)
...
def main():
    ...
    df = pd.read_csv('data/mixed_standard_binary.csv')
    ...
    # df = pd.read_csv("old/spotify_binary.csv")
    ...
    # scores = calculate_causal_scores_DAG(df, nodes, edges, "track_popularity")
